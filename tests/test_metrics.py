...
class MetricsTest(tf.test.TestCase):
    ...
    def test_in_domain_stats_episode(self):
        ...
        self.assertAllClose(in_domain_correct, np.array([[1., 1.],[1., 0.]]))
    ...
    def test_oos_f1_batch(self):
        ...
        [oos_correct],
        [oos_output])
        self.assertAllClose(oos_stats["in_domain_accuracies"], np.array([1.0, 0.5]))
        self.assertAllClose(oos_stats["oos_recalls"], np.array([0., 1.]))
        self.assertAllClose(oos_stats["oos_precisions"], np.array([0. , 0.5]))
        self.assertEqual(oos_stats["in_domain_accuracies"].shape, (2,))
        self.assertEqual(oos_stats["oos_recalls"].shape, (2,))
        self.assertEqual(oos_stats["oos_precisions"].shape, (2,))
